document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('js/processed_data.json')
    ...
    .then(data => {
      ...
      const worstLabels = restaurants.slice(-10).map(restaurant => restaurant.EstablishmentName);
      ...
      const performSearch = () => {
        ...
        const filteredRestaurants = restaurants.filter(restaurant =>
          restaurant.EstablishmentName.toLowerCase().includes(query)
        );
        ...
        filteredRestaurants.forEach(restaurant => {
          ...
          listItem.textContent = `${restaurant.EstablishmentName} - ${restaurant.score}`;
